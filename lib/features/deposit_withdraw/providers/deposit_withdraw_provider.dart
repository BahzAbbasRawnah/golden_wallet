...
class DepositWithdrawProvider extends ChangeNotifier {
  ...
  void _loadMockTransactions() {
    ...
    _transactions = List.generate(10, (index) {
      ...
      // Calculate total
      final total = type == TransactionType.deposit 
          ? amount - fee 
          : amount + fee;
  ...
  Future<bool> processTransaction(TransactionProvider transactionProvider) async {
    ...
    try {
      ...
      final transaction = _currentTransaction!.toTransaction();
      // In a real app, we would call an API to save the transaction
      // For now, we'll just add it to the transaction provider
